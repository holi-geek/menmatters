      ...
      <div className="container px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {cards.map((card, i) => (
            <motion.div
              key={card.title}
              className="bg-card rounded-xl p-8 border border-border hover:border-primary/60 transition-all duration-300 hover:box-glow text-center"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.1, duration: 0.5 }}
            >
              <div className="w-14 h-14 rounded-full gradient-pink flex items-center justify-center mx-auto mb-5">
        ...
        <motion.div
          id="contact"
          className="max-w-xl mx-auto text-center bg-card rounded-2xl p-10 border border-border border-glow"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
        >
          <h3 className="font-heading text-2xl font-bold text-primary text-glow-sm mb-4">
            Get in Touch
          </h3>
